...
/* Brand and platform glyphs for the share sheet.

   The emoji placeholders read as decoration; people scan for the marks they already
   know. WhatsApp and Instagram use their official glyphs, and the generic share button
   shows whichever icon the viewer's own OS uses — the boxed up-arrow on iOS, the linked
...
/** Official Instagram glyph, drawn in its brand gradient. */
export function InstagramIcon({ size = 26, className = "" }: IconProps) {
  // The gradient id has to be unique per render tree or several instances collide.
  const [id] = useState(() => `ig-${Math.random().toString(36).slice(2, 9)}`);
  return (
...
/** Apple's share glyph: a box with an arrow leaving the top. */
export function IosShareIcon({ size = 26, className = "" }: IconProps) {
  return (
...
/** Android's share glyph: three nodes joined by two lines. */
export function AndroidShareIcon({ size = 26, className = "" }: IconProps) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor"
      strokeWidth={1.9} strokeLinecap="round" strokeLinejoin="round" className={className} aria-hidden="true">
...
export function LinkIcon({ size = 26, className = "" }: IconProps) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor"
      strokeWidth={1.9} strokeLinecap="round" strokeLinejoin="round" className={className} aria-hidden="true">
...
export function NativeShareIcon({ size = 26, className = "" }: IconProps) {
  const platform = usePlatform();
  if (platform === "ios") return <IosShareIcon size={size} className={className} />;
  if (platform === "android") return <AndroidShareIcon size={size} className={className} />;
